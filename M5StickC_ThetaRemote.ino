/******************************************************************************
 ...
 Modif. for M5StickC by Masuji Suto 2019/11/10
******************************************************************************/
...
const int buttonPin = M5_BUTTON_HOME;
//const int led1Pin = 4;
//const int led2Pin = 5;
...
#define NEAR_RSSI_THRESHOLD   -60   //[db]
...
void setup() {
  ...
  M5.Lcd.print("RICOH THETA S");
  M5.Lcd.setCursor(10, 30, 2);
  M5.Lcd.print("Remote Control");
  M5.Lcd.setCursor(10, 50, 2);
  M5.Lcd.print("for M5StickC");
  delay(1000);
  ...
  if ( CheckThetaSsid( (const char*)SsidBuff ) == 1 && digitalRead(buttonPin)== 1) {
    ...
    M5.Lcd.fillScreen(TFT_BLACK);
    M5.Lcd.setCursor(10, 10, 2);
    M5.Lcd.print("EEPROM SSID :");
    M5.Lcd.setCursor(10, 25, 2);
    M5.Lcd.print(ssid);
    M5.Lcd.setCursor(10, 40, 2);
    M5.Lcd.print("Wait for connecting...");
  } else {
...
int ConnectTHETA(void)
{
  ...
  while (WiFi.status() != WL_CONNECTED) {
    Serial.print(".");
    M5.Lcd.setCursor(60, 55, 2);
    if(ncnt%2==0) M5.Lcd.print(" * ");
    else M5.Lcd.print("* *");
    ncnt++;
  ...
  if ( iRet != WIFI_SCAN_THETA ) {
    M5.Lcd.fillScreen(TFT_BLACK);
    M5.Lcd.setCursor(10, 20, 2);
    M5.Lcd.print("Theta connected !");
    M5.Lcd.setCursor(10, 40, 2);
    M5.Lcd.print("Press M5 button");
    ncnt = 0;
...
int SearchAndEnterTHETA(void)
{
  ...
  Serial.println("Search THETA");
    ...
    M5.Lcd.fillScreen(TFT_BLACK);
    M5.Lcd.setCursor(10, 30, 2);
    M5.Lcd.print("Search THETA");
    M5.Lcd.setCursor(60, 45, 2);
    if(ncnt%2==0) M5.Lcd.print(" * ");
    else M5.Lcd.print("* *");
    ncnt++;
...
int ThetaAPI_Post_State(void)
{
  ...
  if ( iRet != 0 ) {
    ...
    if (!root.success()) {
      ...
      if(atoi(_recordedTime)==0){
        M5.Lcd.fillScreen(TFT_BLACK);
        M5.Lcd.setCursor(10, 20, 2);
        M5.Lcd.print(strCaptureMode);
        M5.Lcd.setCursor(10, 40, 2);
        M5.Lcd.print("Press M5 button");
        M5.Lcd.setCursor(60, 55, 2);
        }
        ...
        M5.Lcd.setCursor(10, 20, 2);
        M5.Lcd.print("Video recording...");
        M5.Lcd.setCursor(10, 40, 2);
        M5.Lcd.print("Press M5 to stop");
        M5.Lcd.setCursor(60, 55, 2);
        }
      if(ncnt%2==0) M5.Lcd.print(" * ");
      else M5.Lcd.print("* *");
  ...
  if ( iRet != 0 ) {
    ...
    if (!root.success()) {
      ...
      if ( strState.equals("error") ) {
        ...
        M5.Lcd.fillScreen(TFT_BLACK);
        M5.Lcd.setCursor(10, 20, 2);
        M5.Lcd.print("File saved !");        
        M5.Lcd.setCursor(10, 35, 2);
        M5.Lcd.print(String(sFileUri));        
        iTakePicStat = TAKE_PIC_STAT_DONE;
        iRet = 1;
      } else {  // inProgress
        const char* sId = root["id"];
